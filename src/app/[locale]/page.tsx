...
export default async function HomePage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  const teamNames = new Map(regions.flatMap(({ id }) => demoTeams(id).map((team) => [team.id, team.shortName] as const)));
    ...
    <Container maxWidth="xl" sx={{ py: { xs: 4, md: 8 } }}>
      <Box className="vct-grid" sx={{ border: "1px solid var(--vct-border)", borderRadius: 4, p: { xs: 3, md: 6 }, mb: 4, overflow: "hidden", position: "relative" }}>
        <Stack spacing={2} maxWidth={760}>
          ...
          <Typography color="text.secondary" maxWidth={640}>{copy.unpublished}。先用统一规则、等可能系列赛与精确情景聚合搭好计算框架，待管理员录入真实全年赛果后原子发布。</Typography>
      ...
      <Grid container spacing={2} mb={4}>
        {[
          ...
          { icon: <CalendarMonth />, label: copy.published, value: "—", caption: copy.pending },
        ].map((item) => (
